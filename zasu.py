...
class obf:
	...
	def obfuscate(self,file=None,NOTC = "\n\n\t\tOBFUSCATE By SAZXT ft Angung\n\t\tID : %s\n\t\tKEY : %s\n\n",out = ["",False]):
		...
		f.write(BLACK_MAGIC+marshal.dumps(compile(MAC,"zasu","exec"))+NOTC%(id,key))
		f.close()
...
zasu = argparse.ArgumentParser(usage="zasu -i [ filename ] -o [ outfile ]",description="Obfuscate python2")
